...
function MovieCart(movie) {
  ...
  return (
    ...
    src={image_base_url + movie.backdrop_path} alt="movie" className='rounded-lg h-52 w-full object-cover object-right-bottom cursor-pointer'/>
  ...
  {movie?.genres?.slice(0, 2).map(genre => genre.name).join(" | ") || 'No Genre'}.
  {movie?.runtime ? timeFormat(movie.runtime) : 'Unknown Length'}
</p>
   ...
   <div className='flex items-center justify-between mt-4 pb-3'>
    ...
    <p className='flex items-center gap-1 text-sm text-gray-400 mt-1 pr-1'>
      <Star className="w-4 h-4 text-primary fill-primary"/>{23}
    </p>
   </div>
    </div>
